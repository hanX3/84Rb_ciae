...
Sort::Sort(TString raw, TString out, TString name, int number)
{
  ...
  sprintf(temp_file_name, "%s%s_R%04d_m%d_all_fb.root", out.Data(), name.Data(), run_no, WINDOWS);
...
void Sort::Process()
{
  ...
  // write matrix file
  FILE *fp;
  sprintf(temp_file_name, "run%04d_m%d_all_fb.m4b", run_no, WINDOWS);
...
void Sort::FillMatrix()
{
  ...
  for(int i=0;i<hit_num;++i){
    for(int j=0;j<hit_num;++j){
      if(i==j) continue;
      if(FFBBDETCHANNEL[event_channel[i]]){
        matrix->Fill(event_energy[j], event_energy[i]);
      }
      else{
        if(FFBBDETCHANNEL[event_channel[j]]){
